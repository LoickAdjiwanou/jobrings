...
<head>
    <meta charset="utf-8">
    <title>JoBrings - Candidature(s)</title>
                    ...
                    <div class="main_blog text-center roomy-100">
                        <div class="col-sm-8 col-sm-offset-2">
                            <div class="head_title text-center">
                                ...
                                <p>Candidats pour l'offre d'emploi<br/>
                                <?php
                                $re = $bdd -> query('SELECT * FROM publications WHERE id = "'.$id.'"');
                                while($rs = $re->fetch()){
                                    echo $rs['nompublication'];
                                }
                                $re->closeCursor();
                                ?></p>
                            </div>
                        ...
                        <?php    
                            $req = $bdd -> prepare('SELECT * FROM postuler p INNER JOIN chercheur c ON p.nomcandidat = c.nom WHERE nomentreprise = :ne AND id_job = :i');
                            $req->execute(array(
                                'ne'=>$_SESSION['nom'],
                                'i'=>$id
                            ));
                        ...
                        <div class="col-md-4">
                            <div class="blog_item m-top-20" style="border:1px solid lightgrey;box-shadow:3px 3px 3px 3px;padding-top:5px;padding-bottom:0px;padding-left:15px;background-color:white;">
                                ...
                                <?php
                                    echo '<h5><b>Nom:</b> '.$res['nom'].'</h5>';
                                    echo '<h5><b>Prénom:</b> '.$res['prenom'].'</h5>';
                                ?>
                                </div>
                                ...
                                <?php
                                    echo '<h5><b>E-mail:</b> '.$res['email'].'</h5>';
                                    echo '<h5><b>Age:</b> '.$res['age'].'</h5>';
                                    echo '<h5><b>Sexe:</b> '.$res['sexe'].'</h5>';
                                    echo '<h5><b>Situation:</b> '.$res['situation'].'</h5>';
                                    echo '<h5><b>Téléphone:</b> '.$res['telephone'].'</h5>';
                                ?>
                                </div>
                                <div style="padding-bottom: 6px;text-align:left;color:darkblue;">
                                   <?php echo '<h5 style="font-size:15px;"><b><a href="cvs/cv_'.$res['nom'].'.pdf" style="color:orangered;">Visualiser son cv</a></h5>'; ?>
                                </div>
                            </div>
                        </div>
                        <?php
